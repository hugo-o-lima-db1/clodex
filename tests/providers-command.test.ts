...
describe('providers add menu', () => {
  ...
  it('offers ChatGPT OAuth, the OpenAI and OpenCode Go API keys, then a custom server', async () => {
    ...
    expect(options).toEqual(['oauth', 'oauth:antigravity', 'api:openai', 'api:opencode-go', 'api:verboo', 'custom']);
  });
  ...
  it('omits OpenCode Go when a migrated provider retains its built-in template identity', async () => {
    ...
    const options = selectMock.mock.calls[0]?.[0].options.map((option: { value: string }) => option.value);
    expect(options).toEqual(['oauth', 'oauth:antigravity', 'api:openai', 'api:verboo', 'custom']);
